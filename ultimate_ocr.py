#!/usr/bin/env python3
"""
Ultimate OCR extraction based on exact line-by-line analysis
"""
...
def extract_receipt_data_from_google_vision(text: str):
    """Ultimate extraction based on exact line-by-line analysis"""
    try:
        result = {
            'amount': 0.0,
            'transaction_id': '',
        ...
        # Line 30: "Total:" followed by line 32: "10,027.60 ETB"
        for i, line in enumerate(lines):
            if 'Total:' in line and i + 2 < len(lines):
                # Skip the "Status:" line and get the amount
                amount_line = lines[i + 2]
                amount_match = re.search(r'(\d{1,3}(?:,\d{3})*\.?\d*)\s*ETB', amount_line)
                if amount_match:
        ...
        # Line 23: "Transaction Ref:" followed by line 24: "OBTSO"
        for i, line in enumerate(lines):
            if 'Transaction Ref:' in line and i + 1 < len(lines):
        ...
        if not result['transaction_id']:
            for i, line in enumerate(lines):
                if 'FT Ref:' in line and i + 1 < len(lines):
                    result['transaction_id'] = lines[i + 1].strip()
                    break
        ...
        # Line 9: "Transaction Number:" followed by line 13: "CHC85KOLMU"
        if not result['transaction_id']:
            for i, line in enumerate(lines):
                if 'Transaction Number:' in line and i + 4 < len(lines):
                    result['transaction_id'] = lines[i + 4].strip()
                    break
        
        # FIXED: Dashen Bank date - look for Date line
        # Line 21: "Date:" followed by line 18: "Aug 08, 2025 01:07 PM"
        for i, line in enumerate(lines):
            if 'Date:' in line:
                # Look backwards for the date
                for j in range(max(0, i-10), i):
                    if 'Aug' in lines[j] or 'Sep' in lines[j] or 'Oct' in lines[j] or 'Nov' in lines[j] or 'Dec' in lines[j]:
                        result['date'] = lines[j].strip()
                        break
                break
        ...
        # Line 6: "Transaction Time:" followed by line 10: "2025/08/12 13:23:22"
        if not result['date']:
            for i, line in enumerate(lines):
                if 'Transaction Time:' in line and i + 4 < len(lines):
                    result['date'] = lines[i + 4].strip()
                    break
        ...
        # Line 7: "Sender Name:" followed by line 13: "Mariamawit Alemayehu Zewdu"
        for i, line in enumerate(lines):
            if 'Sender Name:' in line and i + 6 < len(lines):
                result['payer'] = lines[i + 6].strip()
                break
        ...
        # Line 8: "Transaction To:" followed by line 12: "Mekonen"
        if not result['payer']:
            for i, line in enumerate(lines):
                if 'Transaction To:' in line and i + 4 < len(lines):
                    result['payer'] = lines[i + 4].strip()
                    break
        
        # FIXED: Receiver patterns
        # Dashen Bank receiver - look for Recipient Name line
        # Line 11: "Recipient Name:" followed by line 16: "Meseret Ayalew"
        for i, line in enumerate(lines):
            if 'Recipient Name:' in line and i + 5 < len(lines):
                result['receiver'] = lines[i + 5].strip()
                break
        ...
        if not result['receiver']:
            for i, line in enumerate(lines):
                if 'Transaction To:' in line and i + 4 < len(lines):
                    result['receiver'] = lines[i + 4].strip()
                    break
