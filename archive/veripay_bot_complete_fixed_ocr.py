...
class VeriPayBot:
    def __init__(self):
        ...
        if VISION_AVAILABLE:
            try:
                self.vision_client = vision.ImageAnnotatorClient(credentials=service_account.Credentials.from_service_account_file("/Users/macbook/veripay/veripay-credentials.json"))
    ...
    async def process_receipt_image(self, update: Update):
        ...
        try:
            ...
            # Convert bytearray to bytes for Google Vision API
            ...
            if self.vision_client:
                try:
                    ...
                    logger.error(f"Image data type: {type(image_bytes)}, size: {len(image_bytes)}")
                    await update.message.reply_text(
