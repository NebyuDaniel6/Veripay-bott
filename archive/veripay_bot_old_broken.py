...
**Need help?** Contact your restaurant admin or Super Admin."""
        ...
        elif query.data == "admin_approve_restaurants":
            if user_role != 'super_admin':
                await query.edit_message_text("❌ Super Admin access required!")
                return
        ...
        elif query.data == "admin_approve_waiters":
            if user_role != 'super_admin':
                await query.edit_message_text("❌ Super Admin access required!")
                return
            
            if 'pending_waiter_approvals' not in pending_restaurant_approvals or not pending_restaurant_approvals['pending_waiter_approvals']:
                await query.edit_message_text("✅ No pending waiter approvals!")
                return
            
            message = "⏳ **Pending Waiter Approvals**\n\n"
            for user_id, waiter_data in pending_restaurant_approvals['pending_waiter_approvals'].items():
                message += f"**User ID:** {user_id}\n"
                message += f"**Waiter Name:** {waiter_data['waiter_name']}\n"
                message += f"**Phone:** {waiter_data['waiter_phone']}\n"
                message += f"**Waiter ID:** {waiter_data['waiter_id']}\n\n"
            ...
            keyboard = []
            for user_id in pending_restaurant_approvals['pending_waiter_approvals'].keys():
                keyboard.append([
                    InlineKeyboardButton(f"✅ Approve Waiter {user_id}", callback_data=f"approve_waiter_{user_id}"),
                    InlineKeyboardButton(f"❌ Reject Waiter {user_id}", callback_data=f"reject_waiter_{user_id}")
                ])
        ...
        elif query.data == "admin_send_updates":
            if user_role != 'super_admin':
                await query.edit_message_text("❌ Super Admin access required!")
                return
            
            await query.edit_message_text("📤 **Send Updates**\n\nThis feature will be implemented to send updates to all users.")
            ...
            await query.edit_message_text("👥 **Manage Waiters**\n\nThis feature will be implemented to manage waiters.")
            ...
            await query.edit_message_text("⏳ **Pending Waiter Approvals**\n\nThis feature will be implemented to show pending waiter approvals.")
        elif query.data == "admin_see_restaurants":
            if user_role != 'super_admin':
                await query.edit_message_text("❌ Super Admin access required!")
                return
            ...
            if approved_restaurants:
                ...
                for user in approved_restaurants:
                    message += f"• {user.get('restaurant_name', 'Unknown')} (ID: {user.get('restaurant_id', 'N/A')})\n"
                message += "\n"
            
            if pending_restaurant_approvals:
                message += "**⏳ Pending Restaurants:**\n"
                for user_id, approval_data in pending_restaurant_approvals.items():
                    if isinstance(approval_data, dict) and 'restaurant_name' in approval_data:
                        message += f"• {approval_data['restaurant_name']} (ID: {approval_data.get('restaurant_id', 'N/A')})\n"
            
            keyboard = [[InlineKeyboardButton("🔙 Back to Admin", callback_data="admin_menu")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(message, reply_markup=reply_markup, parse_mode=Markdown)

        elif query.data == "admin_post_updates":
            if user_role != 'super_admin':
                await query.edit_message_text("❌ Super Admin access required!")
                return
            await query.edit_message_text("📤 **Post Updates**\n\nThis feature allows Super Admin to send system-wide updates to all users.")

        elif query.data == "restaurant_pdf_report":
            if user_role not in ['restaurant_admin', 'super_admin']:
                await query.edit_message_text("❌ Restaurant Admin access required!")
                return
            await query.edit_message_text("📄 **Generate PDF Report**\n\nThis feature will generate a comprehensive PDF report of all restaurant transactions.")

        elif query.data == "restaurant_manage_waiters":
            if user_role not in ['restaurant_admin', 'super_admin']:
                await query.edit_message_text("❌ Restaurant Admin access required!")
                return
            restaurant_id = users.get(query.from_user.id, {}).get('restaurant_id', 'UNKNOWN')
            restaurant_waiters = [user for user in users.values() if user.get('restaurant_id') == restaurant_id and user.get('role') == 'waiter']
            if not restaurant_waiters:
                await query.edit_message_text("👥 **Manage Waiters**\n\nNo waiters registered for this restaurant yet.")
                return
            message = f"👥 **Manage Waiters - Restaurant {restaurant_id}**\n\n"
            for waiter in restaurant_waiters:
                status = "✅ Approved" if waiter.get('status') == 'approved' else "⏳ Pending"
                message += f"• {waiter.get('waiter_name', 'Unknown')} - {status}\n"
            keyboard = [[InlineKeyboardButton("🔙 Back to Admin", callback_data="restaurant_admin_menu")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(message, reply_markup=reply_markup, parse_mode=Markdown)
