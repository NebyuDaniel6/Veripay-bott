...
class VeriPayBot:
    ...
    async def perform_ocr_with_vision(self, image_bytes: bytes) -> str:
        """Enhanced OCR using Google Cloud Vision API with pytesseract fallback"""
        try:
                ...
                try:
                    ...
                    if response.error.message:
                        ...
                        if annotations:
                            ...
                            if text:
                                logger.info(f"Google Vision OCR extracted text: {text[:200]}...")
                                return text
                except Exception as e:
            ...
            if OCR_AVAILABLE:
                try:
                    ...
                    if text:
                        logger.info(f"Pytesseract OCR extracted text: {text[:200]}...")
                        return text
                except Exception as ocr_err:
